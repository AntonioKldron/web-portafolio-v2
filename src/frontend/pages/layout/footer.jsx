...
import { useFooterData } from '@backend/data/footer/footerData';

const Footer = () => {
  const { isDark } = useApp();
  const { data: t, isLoading } = useFooterData();

  if (isLoading || !t) return null; // Invisible while loading
      ...
      <div className="flex flex-col items-center gap-4 relative z-10 text-center">
        ...
        <p className="text-[13px] md:text-[15px] tracking-[0.5em] font-mono uppercase leading-relaxed font-black">
          <span className={isDark 
            ...
            {t.footerText?.nombre || "José Antonio"}
          </span>
        </p>
        ...
        <p className="text-[10px] tracking-[0.3em] font-mono uppercase flex flex-wrap justify-center gap-2 font-bold">
          ...
          <span className={isDark ? "text-slate-400" : "text-white/90"}>
            {t.footerText?.rights || "All Rights Reserved"}
          </span>
          ...
          <span className={isDark 
            ...
            {t.footerText?.rol || "Software Engineer"}
          </span>
        </p>
      </div>
