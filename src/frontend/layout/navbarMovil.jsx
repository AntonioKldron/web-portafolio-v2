...
const Sidebar = () => {
  ...
  const startObserver = useCallback(() => {
    // 1. Limpieza previa del observador existente para evitar duplicados
    ...
    if (window.innerWidth >= 1024 && !scrollContainer) {
      setTimeout(startObserver, 200); // Reintento si el DOM no está listo
      return;
    }
  ...
  useEffect(() => {
    ...
    // 2. RE-CALIBRACIÓN AUTOMÁTICA
    // Escucha cambios de tamaño de ventana (Resize)
    window.addEventListener('resize', startObserver);
    
    // Escucha clics globales (útil si abres menús que desplazan el contenido)
    window.addEventListener('click', () => {
      setTimeout(startObserver, 300); // Re-calibra después de que terminen las animaciones
    });
            ...
            >
              {isActive && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="absolute w-16 h-16 rounded-full border border-indigo-500/50 animate-pulse-ring shadow-[0_0_15px_rgba(99,102,241,0.4)]"></div>
                </div>
              )}
